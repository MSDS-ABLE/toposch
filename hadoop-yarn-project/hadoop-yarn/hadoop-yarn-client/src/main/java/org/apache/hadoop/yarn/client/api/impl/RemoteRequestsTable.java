...
class RemoteRequestsTable<T> implements Iterable<ResourceRequestInfo>{
  ...
  //added by Junqing Xiao 18.1.8.add a mixTpye
  @SuppressWarnings("unchecked")
  ResourceRequestInfo addResourceRequest(Long allocationRequestId,
                                         ...
                                         String labelExpression, MixType mixType, ServicePort port) {
    ...
    if (resourceRequestInfo == null) {
      resourceRequestInfo =
          new ResourceRequestInfo(allocationRequestId, priority, resourceName,
              capability, relaxLocality,mixType,port);
      //todo Junqing Xiao need "put" mixType here?
    ...
    //added by Junqing Xiao 18.1.8.add a mixTpye
    resourceRequestInfo.remoteRequest.setMixType(mixType);

    //AM-Orchestration ZJY
    resourceRequestInfo.remoteRequest.setServicePort(port);
