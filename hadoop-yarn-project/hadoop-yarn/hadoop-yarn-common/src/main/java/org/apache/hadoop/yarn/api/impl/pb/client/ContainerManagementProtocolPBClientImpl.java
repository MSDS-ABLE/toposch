...
public class ContainerManagementProtocolPBClientImpl implements ContainerManagementProtocol,
   ...
   **/
  @Override
  public ServicePort getServicePort(ContainerId containreId) throws YarnException, IOException {
    YarnProtos.ContainerIdProto containerIdProto
            = ((ContainerIdPBImpl) containreId).getProto();
    try {
      return new ServicePortPBImpl(proxy.getServicePort(null, containerIdProto));
    }catch (ServiceException e) {
      RPCUtil.unwrapAndThrowException(e);
      return null;
    }
