...
public class ContainerStatusPBImpl extends ContainerStatus {
  ...
  private static final Logger LOG =
          LoggerFactory.getLogger(ContainerStatusPBImpl.class);
  ...
  //OA-Orchestration ZJY
  private ServicePort serviceport;

  //added by Junqing Xiao 18.1.4
  ...
  //added by Junqing Xiao 18.1.4
  @Override
  public synchronized MixType getMixType() {
    ContainerStatusProtoOrBuilder p = viaProto ? proto : builder;
    if (!p.hasMixType()) {
      return null;
    }
    return convertFromProtoFormat(p.getMixType());
  }

  @Override
  public synchronized void setMixType(MixType mixType) {
    maybeInitBuilder();
    if (mixType == null) {
      builder.clearMixType();
      return;
    }
    builder.setMixType(convertToProtoFormat(mixType));
  }
  ...
  public ServicePort getServicePort() {
    ...
    return convertFromProtoFormat(p.getServicePort());
  }
  ...
  public void setServicePort(ServicePort serviceport){
    ...
    builder.setServicePort(converToProtoFormat(serviceport));
  }
  ...
  //AM-Orchestration
  private ServicePortProto converToProtoFormat(ServicePort SP){
    return ProtoUtils.convertToProtoFormat(SP);
  }
  private ServicePortPBImpl convertFromProtoFormat(ServicePortProto servicePortProto){
    return new ServicePortPBImpl(servicePortProto);
  }
