...
import org.apache.hadoop.yarn.api.records.impl.pb.ServicePortPBImpl;
...
public class ContainerManagementProtocolPBServiceImpl implements ContainerManagementProtocolPB  {
  ...
  public ServicePortProto getServicePort(RpcController arg0, ContainerIdProto proto)
          throws ServiceException {
    ContainerIdPBImpl request = new ContainerIdPBImpl(proto);
    try{
      ServicePort response = real.getServicePort(request);
      return ((ServicePortPBImpl)response).getProto();
    }catch (YarnException e) {
      throw new ServiceException(e);
    } catch (IOException e) {
      throw new ServiceException(e);
