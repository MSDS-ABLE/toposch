...
public class ContainerImpl implements Container {
  ...
  //add mixType here, default value is OFFLINE.  added by Junqing Xiao 18.1.2
  ...
  //added by Junqing Xiao 18.1.2
  public ContainerImpl(Configuration conf, Dispatcher dispatcher,
                       ContainerLaunchContext launchContext, Credentials creds,
                       NodeManagerMetrics metrics,
                       ContainerTokenIdentifier containerTokenIdentifier, Context context,
                       long startTs,MixType type) {
      this(conf, dispatcher, launchContext, creds, metrics, containerTokenIdentifier, context, startTs);
      this.setMixType(type);
  }
  ...
  public ContainerImpl(Configuration conf, Dispatcher dispatcher,
                       ...
                       NodeManagerMetrics metrics,
                       ContainerTokenIdentifier containerTokenIdentifier, Context context,
                       long startTs,MixType type,ServicePort servicePort) {
      this(conf, dispatcher, launchContext, creds, metrics, containerTokenIdentifier, context, startTs, type);
  ...
  private static StateMachineFactory
    ...
    .addTransition(ContainerState.RUNNING,
        ContainerState.LISTENING,
        ContainerEventType.CONTAINER_RUNNING_LISTENING)

    /**
     * ADD BY ZJY
     * To ensure the statemachine can run normally, expecilly for UPDATE_DIAGNOSTICS_MSG
     * */
    .addTransition(ContainerState.LISTENING,
            ContainerState.LISTENING,
            EnumSet.of(
            ContainerEventType.RESOURCE_LOCALIZED,
            ContainerEventType.RESOURCE_FAILED,
  ...
  public org.apache.hadoop.yarn.api.records.ContainerState getCurrentState() {
    ...
    /**
     * ADD BY ZJY
     * when RM receiving the LISTENING state, the update RMContainer StateMachine from RMNODE info
     * once compelete(include default state) state received by RM , it will send Kill_container(?) order
     * */
    case LISTENING:
      return org.apache.hadoop.yarn.api.records.ContainerState.RUNNING;
    case DONE:
  ...
  public ContainerStatus cloneAndGetContainerStatus() {
    ...
    try {
      ...
      //LOG.info("###ZJY justdebug : mix_type is " + mixType + ",serviceport is " + ServicePort.getServicePort());
      status.setIPs(ips == null ? null : Arrays.asList(ips.split(",")));
      status.setHost(host);
      status.setMixType(mixType);
      status.setServicePort(ServicePort);
      return status;
    } finally {
      this.readLock.unlock();
    }
  ...
  //junqing Xiao 18.1.2.setter of mixType here
  //todo junqing Xiao need getter?
  private void setMixType(MixType type) {
    this.mixType = type;
  }
  ...
  public void handle(ContainerEvent event) {
    try {
      ...
      /***
       * ADD BY ZJY
      judeg which containerkillevent exit code and diag
       */
      if(event instanceof  ContainerKillEvent){
        LOG.info("###ZJY diag info" +((ContainerKillEvent)event).getDiagnostic());
        LOG.info("###ZJY exit code is "  +
        ((ContainerKillEvent)event).getContainerExitStatus());
      }
