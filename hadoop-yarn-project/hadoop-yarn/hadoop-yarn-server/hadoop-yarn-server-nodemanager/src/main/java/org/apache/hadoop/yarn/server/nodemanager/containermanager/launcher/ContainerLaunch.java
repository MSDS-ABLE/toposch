...
public class ContainerLaunch implements Callable<Integer> {
  ...
  public Integer call() {
    ...
    try {
      ...
      ret = launchContainer(new ContainerStartContext.Builder()
          ...
          .setContext(context).build());//AM-Orchestration ZJY : add nmcontext
  ...
  protected void handleContainerExitCode(int exitCode, Path containerLogDir) {
    ...
    if (exitCode == ExitCode.FORCE_KILLED.getExitCode()
      ...
      /**
       * ADD by ZJY
       * when container executed and exit with success, put the Container_RUNNING_LISTENING
       * instead of CONTAINER_EXITED_WITH_SUCCESS, so that clean_up operation cannot be sent
       * */
      //dispatcher.getEventHandler().handle(
        // new ContainerListeningEvent(containerId));
    }
  }
