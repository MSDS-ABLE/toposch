...
char* sanitize_docker_command(const char *line) {
  static struct option long_options[] = {
    ...
    {"add-host", required_argument, 0, 's' },
    //added by junqing xiao 18.4.9
    {"expose", required_argument, 0, 'z' },
    {0, 0, 0, 0}
  };
  ...
  strcat(output, " ");
  optind = 1;
  while((c=getopt_long(split_counter, linesplit, "dv:e:m:", long_options, &option_index)) != -1) {
    switch(c) {
      case 'm':      //AM-Orchestration ZJY, check port map
        quote_and_append_arg(&output, &output_size, "-p ", optarg);
        break;
      case 'n':
      ...
      case 'v':
        ...
        //added by junqing xiao 18.1.16
      case 'e':
        quote_and_append_arg(&output, &output_size, "-e ", optarg);
        break;
        //added by junqing xiao 18.1.17
      case 's':
        quote_and_append_arg(&output, &output_size, "--add-host=", optarg);
        break;
        //added by junqing xiao 18.4.9
      case 'z':
        quote_and_append_arg(&output, &output_size, "--expose=", optarg);
        break;
      case 'a':
