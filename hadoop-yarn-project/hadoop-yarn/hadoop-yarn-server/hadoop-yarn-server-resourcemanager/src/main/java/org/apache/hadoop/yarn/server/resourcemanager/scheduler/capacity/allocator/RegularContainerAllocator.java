...
public class RegularContainerAllocator extends AbstractContainerAllocator {
  ...
  private boolean checkBatchAvoid(NodeId nodeId,NodeType type,SchedulerRequestKey schedulerKey){
    //检查flag
    if ( !rmContext.getBatchAvoidManager().getEnableFlag() ){
      return true;
    }
    LOG.info("start batch avoid");
    // 开始离线避让
    if ( schedulerKey.getMixType() == MixType.ONLINE ){
      return true;
    }
    Random random = new Random();
    double randVal = random.nextDouble();
    if ( randVal < rmContext.getBatchAvoidManager().getNodeScore(nodeId,type) ) {
      LOG.warn("**junqing xiao** batch avoid!!! nodeId = " +
              nodeId + " type = " + type + " opportunity = " + application.getSchedulingOpportunities(schedulerKey));
      return false;
    }else {
      return true;
    }
  ...
  private boolean canAssign(SchedulerRequestKey schedulerKey,
    ...
    if (type == NodeType.OFF_SWITCH) {
      LOG.info("**junqing xiao** canAssign OFF_SWITCH 1");
      ...
      if (application.getCSLeafQueue().getRackLocalityAdditionalDelay() > -1) {
        LOG.info("**junqing xiao** canAssign OFF_SWITCH getActualRackLocalityDelay() = " + getActualRackLocalityDelay() +
        ...
        LOG.info("**junqing xiao** canAssign OFF_SWITCH getNumClusterNodes() = " + rmContext.getScheduler().getNumClusterNodes() +
                "requiredContainers = " + requiredContainers + "localityWaitFactor = " + localityWaitFactor +
                "missedOpportunities = " + missedOpportunities);
        return (Math.min(rmContext.getScheduler().getNumClusterNodes(),
    ...
    if (type == NodeType.RACK_LOCAL) {
      ...
      LOG.info("**junqing xiao** canAssign RACK_LOCAL");
      long missedOpportunities =
          application.getSchedulingOpportunities(schedulerKey);
      LOG.info("**junqing xiao** canAssign RACK_LOCAL getActualNodeLocalityDelay() = " + getActualNodeLocalityDelay() +
              "missedOpportunities = " + missedOpportunities);
  ...
  private ContainerAllocation assignNodeLocalContainers(
    ...
    if (canAssign(schedulerKey, node, NodeType.NODE_LOCAL, reservedContainer)
            && checkBatchAvoid(node.getNodeID(),NodeType.NODE_LOCAL,schedulerKey)) {
  ...
  private ContainerAllocation assignRackLocalContainers(
    ...
    if (canAssign(schedulerKey, node, NodeType.RACK_LOCAL, reservedContainer)
            && checkBatchAvoid(node.getNodeID(),NodeType.RACK_LOCAL,schedulerKey)) {
      return assignContainer(clusterResource, node, schedulerKey,
  ...
  private ContainerAllocation assignOffSwitchContainers(
    ...
    if (canAssign(schedulerKey, node, NodeType.OFF_SWITCH, reservedContainer)
            && checkBatchAvoid(node.getNodeID(),NodeType.OFF_SWITCH,schedulerKey)) {
      return assignContainer(clusterResource, node, schedulerKey,
  ...
  private ContainerAllocation assignContainersOnNode(Resource clusterResource,
    ...
    LOG.info("**junqing xiao** data-local");
    // Data-local
    ...
    if (nodeLocalAsk.getCount() > 0) {
      ...
      if (Resources.greaterThan(rc, clusterResource,
        ...
        LOG.info("**junqing xiao** data-local return state = " + allocation.state);
        return allocation;
      }
    }

    LOG.info("**junqing xiao** rack-local");
    // Rack-local
    ...
    if (rackLocalAsk.getCount() > 0) {
      ...
      if (Resources.greaterThan(rc, clusterResource,
          allocation.getResourceToBeAllocated(), Resources.none())) {
        allocation.requestLocalityType = requestLocalityType;
        LOG.info("**junqing xiao** rack-local return state = " + allocation.state);
        return allocation;
      }
    }

    LOG.info("**junqing xiao** off-switch");
    ...
    if (offSwitchAsk.getCount() > 0) {
      ...
      allocation.requestLocalityType = requestLocalityType;
      LOG.info("**junqing xiao** off-switch return state = " + allocation.state);
      return allocation;
    }
    ActivitiesLogger.APP.recordSkippedAppActivityWithoutAllocation(
        activitiesManager, node, application, priority,
  ...
  private ContainerAllocation assignContainer(Resource clusterResource,
    ...
    LOG.info("**junqing xiao** capability = " + capability + " available = " + available +
  ...
  ContainerAllocation doAllocation(ContainerAllocation allocationResult,
    ...
    if (reservedContainer == null) {
      ...
      if (allocationResult.containerNodeType != NodeType.OFF_SWITCH) {
        //if (LOG.isDebugEnabled()) {
          LOG.info("Resetting scheduling opportunities to 0,schedulerKey = " + schedulerKey);
        //}
  ...
  private ContainerAllocation allocate(Resource clusterResource,
    ...
    Iterator<FiCaSchedulerNode> iter = schedulingPS.getPreferredNodeIterator(
        ps);
    LOG.info("**junqing xiao** : before iter ");
    while (iter.hasNext()) {
      FiCaSchedulerNode node = iter.next();
      LOG.info("**junqing xiao** : before try node = " + node);
    ...
    LOG.info("**junqing xiao** : after iter ");
    return result;
  }
