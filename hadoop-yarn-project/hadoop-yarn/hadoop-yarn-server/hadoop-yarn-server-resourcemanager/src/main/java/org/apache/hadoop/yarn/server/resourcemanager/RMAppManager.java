    ...
    if (!submissionContext.getUnmanagedAM()) {
      ...
      if (amReqs == null || amReqs.isEmpty()) {
        if (submissionContext.getResource() != null) {
          if ( MixType.ONLINE.toString()
                  .equals(submissionContext.getAMContainerSpec().getEnvironment().get("YARN_CONTAINER_MIX_TYPE")) ){
            LOG.info("**junqing xiao** YARN_CONTAINER_MIX_TYPE == online");
            ResourceRequest resourceRequest = BuilderUtils
                    .newResourceRequest(RMAppAttemptImpl.AM_CONTAINER_PRIORITY,
                            ResourceRequest.ANY, submissionContext.getResource(), 1);
            resourceRequest.setMixType(MixType.ONLINE);
            amReqs = Collections.singletonList(resourceRequest);
          }else {
