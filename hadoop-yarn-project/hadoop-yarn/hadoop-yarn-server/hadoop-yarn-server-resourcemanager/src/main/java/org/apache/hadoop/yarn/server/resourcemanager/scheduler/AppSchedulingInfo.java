...
public class AppSchedulingInfo {
  ...
  public PendingAsk getPendingAsk(SchedulerRequestKey schedulerKey,
    ...
    try {
      this.readLock.lock();
      LOG.info("**junqing xiao** show schedulerKeyToPlacementSets");
      for (Map.Entry<SchedulerRequestKey, SchedulingPlacementSet<SchedulerNode>> entry:schedulerKeyToPlacementSets.entrySet()){
        LOG.info("**junqing xiao**  " + entry.getKey() + ":::" + entry.getValue());
      }
      LOG.info("**junqing xiao** end show schedulerKeyToPlacementSets");
