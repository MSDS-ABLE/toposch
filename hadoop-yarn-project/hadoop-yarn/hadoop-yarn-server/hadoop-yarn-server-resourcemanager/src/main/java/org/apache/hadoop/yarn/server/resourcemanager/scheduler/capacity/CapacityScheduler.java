  ...
  private void allocateContainersToNode(NodeId nodeId,
    ...
    if (null != node) {
      ...
      if (null != assignment && withNodeHeartbeat) {
        ...
        //added by junqing xiao 18.10.10
        final int MAX_ALLOCATION_PER_ROUND = 50;
        int num = 0;

        while (canAllocateMore(assignment, offswitchCount, assignedContainers)
                && num < MAX_ALLOCATION_PER_ROUND) {
          //added by junqing xiao 18.5.29
          try {
            readLock.lock();
            //Map<NodeId,Double> nodeScoreMap = this.rmContext.getBatchAvoidManager().getNodeScores();
//            StringBuilder builder = new StringBuilder("**nodeScores**  {");
//            for (Map.Entry<NodeId,Double> entry : nodeScoreMap.entrySet()){
...
//            if ( randVal < nodeScoreMap.get(nodeId)  ){
//              LOG.warn("batch avoid!!! nodeId = " + nodeId);
//              break;
//            }
          }finally {
            readLock.unlock();
